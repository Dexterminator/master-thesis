...
The memory usage grows in a similar manner to the tiny dataset, though the values are greater.
...
The post processing step takes up about 6.5\% of the execution time for 8 workers, close to the medium dataset.

Without the parallel processing, the large dataset shows slighlty better speedup for 8 workers. However, this increase is only from 4.3X to 4.6X
speedup, meaning that it still has relatively low efficiency.
